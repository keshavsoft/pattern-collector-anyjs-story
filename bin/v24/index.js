import { extractRegex, fileNamesJson } from "pattern-collector-base-files";
...
const guardFileType = ({ fileType }) => {
    const fileTypesKnowledge = fileNamesJson();

    return fileType in fileTypesKnowledge;
};

const startFunc = ({ fileContent, fileType }) => {
    const isFileTypeValid = guardFileType({ fileType });

    if (!isFileTypeValid) return false;
